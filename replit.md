...
- **Design System**: "new-york" style variant for shadcn/ui, Noto Sans KR (Korean), Inter (English) fonts. **Reference-driven mood-board styleguide** — all sections below the hero share a fixed pink-desert photographic backdrop (`attached_assets/optimized/pink-desert-bg.webp` injected via `.page-backdrop` element in `home.tsx`) with a soft white→coral overlay. Mint sage (#6EC9A3) retained for hero CTA + brand identity; coral palette (--color-coral #E8B5A8, --color-coral-deep #C77965, --color-coral-soft #F5DDD4) drives badges, accents, focus rings on marketing sections. Card system mixes `.glass-frost` (translucent white + 20px backdrop-blur + soft coral shadow) with `.ink-card` (solid #1A1A1A white text) for high-contrast layered floating compositions. **Section compositions** mirror the reference's asymmetric overlapping cards — About (light glass FOLLOW left + tall white "15Y" feature right + small dark "검증된 합격" reference bottom), Achievements (7-col dark feature + 5-col 3-glass-stack), Portfolio (tall white feature + dark mid + small glass), Testimonials (tall dark quote + glass review + small white review). Circular pill arrow buttons (`.pill-arrow`, `.pill-arrow-wide`) replace rectangular CTAs site-wide. Mobile-first responsive (cards stack vertically on `<lg`). Footer is solid ink with coral accent labels.
