...
- 걷어낸 블록의 이미지(첫 팝업)는 서버가 `<head>`에 `<link rel="preload" as="image">`로 되살린다(빌드에 파일이 있을 때만). 이것이 없으면 캐시 없는 첫 방문에서 첫 팝업 이미지가 갤러리 사진과 대역을 나눠 첫 팝업이 LTE 4.2초·느린 4G 21초 늦게 떴다(2026-09-11 속도 제한 실측, 복구 후 둘 다 1.0초). `fetchpriority="high"`는 효과 차이가 없어 넣지 않았다(앱 스크립트와 대역 경쟁 방지).
...
| 네이버 소유확인 | **완료(2026-08-28)** — 메타코드 삽입·서치어드바이저 소유확인·사이트맵 제출까지 형님이 마침 | 색인 반영은 네이버 수집 주기 대기 |
| fb:app_id·GA 측정ID | 플레이스홀더 잔존 | client/index.html — GA는 형님 계정에서 측정ID 발급 필요, fb:app_id는 미사용이면 줄 삭제 후보 |
