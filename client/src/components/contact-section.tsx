...
const onFocus = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
  ...
  e.currentTarget.style.boxShadow = '0 0 0 3px rgba(139,92,246,0.12), inset 0 1px 3px rgba(0,0,0,0.05)';
...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="py-20" style={{ background: 'rgba(255,255,255,0.18)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-16">
          ...
          <div className="glass-card p-8">
            <h3 className="text-lg font-bold mb-6" style={{ color: 'rgba(0,0,0,0.82)' }}>무료 체험수업 신청</h3>
            <form onSubmit={form.handleSubmit((d) => submitConsultation.mutate(d))} className="space-y-5">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input placeholder="학생 이름을 입력해주세요" {...form.register("studentName")} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="tel" placeholder="연락 가능한 번호" {...form.register("phone")} style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
                ...
                <div>
                  ...
                  <select style={{ ...inputStyle, appearance: 'none' }} onFocus={onFocus} onBlur={onBlur}
                    ...
                    <option value="" disabled>과정을 선택해주세요</option>
                    {['입시미술 (디자인/만화)','디지털 드로잉','중등미술','상담 후 결정'].map(c => (
                      <option key={c} value={c}>{c}</option>
                    ))}
                  </select>
                  {form.formState.errors.course && (
                    <p className="text-red-500 text-xs mt-1">{form.formState.errors.course.message}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <textarea rows={4} placeholder="궁금한 점을 자유롭게 작성해주세요" {...form.register("message")}
          ...
          {/* Info */}
          <div>
            <h3 className="text-lg font-bold mb-6" style={{ color: 'rgba(0,0,0,0.82)' }}>학원 정보</h3>
            ...
            <div className="space-y-4 mb-8">
              {[
                ...
                <div key={title} className="glass-card flex items-start gap-4 p-5">
                  ...
                  <div>
                    <h4 className="font-semibold text-sm mb-1" style={{ color: 'rgba(0,0,0,0.78)' }}>{title}</h4>
                    {lines.map((l, i) => (
                      <p key={i} className="text-sm" style={{ color: i === 0 ? 'rgba(0,0,0,0.62)' : 'rgba(0,0,0,0.42)' }}>{l}</p>
                    ))}
                  </div>
                </div>
              ))}
              ...
              <div className="flex">
                {(['masan', 'gimhae'] as const).map((campus) => (
                  <button key={campus} onClick={() => setSelectedCampus(campus)}
                    ...
                    style={{
                      background: selectedCampus === campus ? 'linear-gradient(135deg, rgba(139,92,246,0.15), rgba(6,182,212,0.15))' : 'transparent',
                      color: selectedCampus === campus ? '#8B5CF6' : 'rgba(0,0,0,0.45)',
                      borderBottom: selectedCampus === campus ? '2px solid #8B5CF6' : '2px solid transparent',
                    }}>
