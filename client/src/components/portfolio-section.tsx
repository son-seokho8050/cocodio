...
export default function PortfolioSection() {
  return (
    <section id="portfolio" className="py-20" style={{ background: 'rgba(255,255,255,0.18)' }}>
            ...
            <a href={card.link} key={card.id} target="_blank" rel="noopener noreferrer">
              <div className="glass-card group cursor-pointer p-8 text-center">
                <div
                  ...
                  style={{ background: `linear-gradient(135deg, ${card.color}22, ${card.color}44)`, border: `1px solid ${card.color}33` }}
                >
                  <card.icon className="h-8 w-8" style={{ color: card.color }} />
                </div>
                <h3 className="text-lg font-bold mb-3" style={{ color: 'rgba(0,0,0,0.82)' }}>
                  {card.title}
                </h3>
                <p className="mb-6 text-sm leading-relaxed" style={{ color: 'rgba(0,0,0,0.52)' }}>
                  {card.description}
                </p>
