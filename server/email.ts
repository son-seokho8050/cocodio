...
const JONGHAP_NOTIFY_EMAIL = 'wookyung8140@gmail.com';
...
export async function sendJonghapInquiryNotification(data: JonghapInquiryData): Promise<boolean> {
  const submittedAt = data.createdAt
    ? new Date(data.createdAt).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })
    : '-';
  ...
  console.log(`🎓 학년: ${data.grade}`);
  console.log(`📞 연락처: ${data.phone}`);
  console.log(`🏫 지원 희망: ${data.targetUniversity || '-'}`);
  console.log(`⏰ 신청 시간: ${submittedAt}`);
  console.log(`💬 문의 내용: ${data.message || '별도 문의사항 없음'}`);
  console.log('================================');
  ...
  try {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; border-radius: 12px; overflow: hidden;">
        <div style="background: #1A1A1A; padding: 24px 32px;">
          <h1 style="color: white; margin: 0; font-size: 20px;">🎓 학생부종합전형 문의가 접수되었습니다</h1>
          <p style="color: #E8B5A8; margin: 6px 0 0; font-size: 14px;">서울대 총원장 직강 · 학종전형 문의 알림</p>
        </div>
        <div style="padding: 32px; background: white;">
          <table style="width: 100%; border-collapse: collapse; font-size: 15px;">
            <tr style="border-bottom: 1px solid #e5e7eb;">
              <td style="padding: 12px 8px; color: #6b7280; width: 130px;">학생 이름</td>
              <td style="padding: 12px 8px; font-weight: 600; color: #111827;">${data.studentName}</td>
            </tr>
            <tr style="border-bottom: 1px solid #e5e7eb;">
              <td style="padding: 12px 8px; color: #6b7280;">학년</td>
            ...
            <tr style="border-bottom: 1px solid #e5e7eb;">
              <td style="padding: 12px 8px; color: #6b7280;">지원 희망 대학·학과</td>
              <td style="padding: 12px 8px; color: #111827;">${data.targetUniversity || '-'}</td>
            </tr>
            <tr style="border-bottom: 1px solid #e5e7eb;">
              <td style="padding: 12px 8px; color: #6b7280;">문의 내용</td>
              <td style="padding: 12px 8px; color: #111827; white-space: pre-line;">${data.message || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 12px 8px; color: #6b7280;">신청 시각</td>
              <td style="padding: 12px 8px; color: #111827;">${submittedAt}</td>
    ...
    await transporter.sendMail({
      from: `"코코미술학원" <${process.env.GMAIL_EMAIL}>`,
      to: JONGHAP_NOTIFY_EMAIL,
      subject: `[학종전형 문의] ${data.studentName} (${data.grade})`,
      html,
    });

    console.log(`✅ 학종 문의 이메일 발송 완료 → ${JONGHAP_NOTIFY_EMAIL}`);
    return true;
  } catch (error) {
    console.error('⚠️ 학종 문의 이메일 발송 실패:', error);
    return true;
  }
}
